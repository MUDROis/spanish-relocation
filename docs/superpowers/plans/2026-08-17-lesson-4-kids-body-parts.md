...
```html
  ...
  <section class="block" style="--c:#D62828">
    ...
    <div class="script">
      ...
      <em>estar</em> (быть) + какой ты: <em>Estoy mal</em> (мне плохо). Вспомни SER vs ESTAR из Урока 3!
    </div>
    ...
    <div class="script">
      ...
      Если тебе совсем плохо: <em>Estoy mal → Me duele todo</em>. Эмоции живут в теле!
    </div>
  </section>
...
const NURSE_QUESTION={es:'¿Dónde te duele?', ru:'Где у тебя болит? (вопрос медсестры)'};
const RANDOM_BODY=[
...
function speakSeq(list){
  ...
  u.onend=function(){
    if(list[1]){
      const v=new SpeechSynthesisUtterance(list[1]);
      v.lang='es-ES';v.rate=0.8;v.pitch=1.05;
      if(esVoice)v.voice=esVoice;
...
function pressRandomBody(){
  ...
  out.innerHTML='<div class="es">'+NURSE_QUESTION.es+'</div><div class="ru">'+NURSE_QUESTION.ru+'</div>'
    +'<div class="es">'+m.es+'</div><div class="ru">'+m.ru+'</div>';
  speakSeq([NURSE_QUESTION.es,m.es]);
}
...
const SIMON_ROUNDS=[
  {cmd:'¡Simón dice: toca los ojos!', ru:'Симон говорит: тронь глаза!', target:'ojos', trap:false},
  ...
  {cmd:'¡Toca la boca!', ru:'Тронь рот! (Симон НЕ говорил!)', target:'boca', trap:true},
  {cmd:'¡Simón dice: toca la rodilla!', ru:'Симон говорит: тронь колено!', target:'rodilla', trap:false}
];
...
const SYMPTOMS=[
  ...
  {emoji:'😖', es:'Estoy mal', ru:'Мне плохо', gender:false},
...
const DOC_ROUNDS=[
  {doc:'¿Qué te pasa?', docRu:'Что с тобой?', correct:'Me duele la cabeza.', distract:['Me llamo Marta','Soy de Rusia']},
